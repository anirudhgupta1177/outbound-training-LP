...
// One phase of the micro-course — the counterpart of a Mastery lesson page.
export default function MicroCoursePhase() {
  ...
  // Unknown or removed phase: fall back to the overview rather than a blank page.
  if (parts.length > 0 && !part) {
    return <Navigate to="/micro-course" replace />;
  }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 py-20 text-center">
          <p className="text-gray-400 mb-6">This phase isn&apos;t available.</p>
          <Link to="/micro-course" className="text-amber-400 hover:underline">
            ← Back to the micro-course
          </Link>
        </main>
      </div>
    );
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="mb-6">
          <p className="text-sm font-semibold text-amber-400 mb-2">
            Phase {index + 1} of {parts.length}
          </p>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-3">
            {part.title.replace(/^Phase\s*\d+\s*[—–-]\s*/i, '')}
          </h1>
        ...
        {materials.length > 0 && (
          <section className="mb-8">
            <h2 className="text-xl font-bold text-white mb-4">Resources for this phase</h2>
        ...
        <nav className="flex items-stretch gap-3 sm:gap-4 pt-6 border-t border-gray-800">
          {prev ? (
            ...
            >
              <HiArrowLeft className="w-5 h-5 text-gray-500 group-hover:text-amber-400 flex-shrink-0 transition-colors" />
              <span className="min-w-0">
                <span className="block text-xs text-gray-500">Previous</span>
                <span className="block text-sm font-medium text-white truncate">
                  Phase {index}: {prev.title.replace(/^Phase\s*\d+\s*[—–-]\s*/i, '')}
                </span>
              </span>
            </Link>
          ) : (
          ...
          {next ? (
            ...
            >
              <span className="min-w-0">
                ...
                <span className="block text-sm font-medium text-white truncate">
                  Phase {index + 2}: {next.title.replace(/^Phase\s*\d+\s*[—–-]\s*/i, '')}
                </span>
              </span>
              <HiArrowRight className="w-5 h-5 text-gray-500 group-hover:text-amber-400 flex-shrink-0 transition-colors" />
            </Link>
          ) : (
            bookingUrl && (
              <a
                ...
                className="flex-1 min-w-0 flex items-center justify-end gap-3 p-4 rounded-xl border border-purple-500/25 bg-gradient-to-br from-[#15101f] to-[#111111] hover:border-purple-500/50 transition-all group text-right"
              >
                <span className="min-w-0">
                  <span className="block text-xs text-gray-500">Finished the marathon?</span>
                  <span className="block text-sm font-medium text-white truncate">
