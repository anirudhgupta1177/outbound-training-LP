...
/* ------------------------------------------------------------------ */
/*  Testimonial Card - uniform structure, forced 16:9 video            */
/* ------------------------------------------------------------------ */
const TestimonialCard = memo(function TestimonialCard({ testimonial, onPlay }) {
  return (
    <div className="testimonial-card bg-[#0F0F14] rounded-2xl overflow-hidden h-full flex flex-col"
      style={{ border: '1px solid rgba(34, 211, 238, 0.15)', minHeight: '480px' }}
    >
      {/* Video wrapper - FORCED 16:9, object-fit: cover */}
      <div
        className="video-wrapper relative cursor-pointer group bg-black"
      ...
      >
        <video
          ...
          style={{
            objectFit: 'cover',
            objectPosition: testimonial.videoPosition || 'center',
          }}
        />
      ...
      <div className="p-5 flex flex-col flex-1 gap-2.5">
        ...
        <p className="text-white font-semibold leading-[1.4]"
          style={{
            fontSize: '16px',
            display: '-webkit-box',
            WebkitLineClamp: 3,
        ...
        <p className="mt-auto leading-snug"
          style={{
            fontSize: '14px',
            lineHeight: 1.5,
            color: '#9CA3AF',
    ...
    >
      <div className="max-w-lg mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6 sticky top-0 bg-dark/95 backdrop-blur-lg py-3 z-10">
          ...
          <button onClick={onClose} className="text-white/80 hover:text-white p-2" aria-label="Close">
            <HiX className="w-6 h-6" />
          </button>
        </div>
        <div className="space-y-4">
          {testimonials.map((t, i) => (
            <TestimonialCard key={i} testimonial={t} onPlay={onPlay} />
          ))}
        </div>
      </div>
    </motion.div>
...
function Testimonials() {
  ...
  const handleScroll = useCallback(() => {
    ...
    setActiveIndex(Math.min(idx, videoTestimonials.length - 1));
  }, []);
  ...
  const scrollTo = useCallback((direction) => {
    ...
    const newIndex = direction === 'next'
      ? Math.min(activeIndex + 1, videoTestimonials.length - 1)
      : Math.max(activeIndex - 1, 0);
    el.scrollTo({ left: newIndex * (cardWidth + 16), behavior: 'smooth' });
  ...
  return (
    <section id="testimonials" className="py-8 md:py-20 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center justify-center divide-x divide-white/10">
            <StatCard target={1132} suffix="+" label="Students" duration={1500} inView={statsInView} reduced={prefersReducedMotion} />
            <StatCard target={7847} suffix="" label="Meetings Booked" duration={1500} inView={statsInView} reduced={prefersReducedMotion} />
            {isIndia ? (
              <StatCard target={4.2} suffix="Cr+" label="Pipeline Generated" duration={1200} inView={statsInView} reduced={prefersReducedMotion} />
            ) : (
              <div className="flex-1 text-center px-2">
                <p className="text-xl sm:text-2xl md:text-3xl font-display font-bold gradient-text leading-none">{revenueDisplay}+</p>
        ...
        {/* NO FILTER PILLS - removed entirely */}
        ...
        <div className="md:hidden mt-6">
          ...
          >
            {videoTestimonials.map((testimonial, index) => (
              ...
              >
                <TestimonialCard testimonial={testimonial} onPlay={openVideo} />
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex items-center justify-between px-4">
            ...
            <CarouselDots total={videoTestimonials.length} active={activeIndex} />
            <span className="text-text-muted text-xs tabular-nums">
              {activeIndex + 1} / {videoTestimonials.length}
            </span>
          </div>
        </div>

        {/* DESKTOP: 3-column grid - ALL testimonials, no filtering */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="hidden md:grid mt-8 md:mt-10 gap-6"
          style={{
      ...
      {/* Modals */}
      <AnimatePresence>
        {videoModal && <VideoModal testimonial={videoModal} onClose={closeVideo} />}
      </AnimatePresence>
      <AnimatePresence>
        {allStoriesModal && (
          <AllStoriesModal
...
export const VideoCard = TestimonialCard;
